...
async fn test_thesis_locking_flow() -> Result<()> {
    ...
    note_area
        .send_keys("E2E Institutional Thesis - Quality Compounder")
        .await?;
...
async fn test_persistence_ui_controls() -> Result<()> {
    ...
    let import_btn = ctx
        .driver
        .query(By::ClassName("open-analysis-btn"))
        .first()
        .await?;
    assert!(import_btn.is_displayed().await?);
    assert_eq!(
        import_btn.attr("title").await?,
        Some("Open Analysis from File".to_string())
    );

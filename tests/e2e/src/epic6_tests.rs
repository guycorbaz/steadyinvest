...
async fn test_complete_analyst_workflow() -> Result<()> {
    ...
    let chart_container = ctx
        .driver
        .query(By::ClassName("ssg-chart-container"))
        .wait(Duration::from_secs(10), Duration::from_millis(500))
        .first()
        .await?;
    ...
    let slider_controls = ctx
        .driver
        .find(By::ClassName("chart-slider-controls"))
        .await?;
    ...
    let quality = ctx
        .driver
        .query(By::ClassName("quality-dashboard"))
        .wait(Duration::from_secs(10), Duration::from_millis(500))
        .first()
        .await?;
    ...
    let valuation = ctx
        .driver
        .query(By::ClassName("valuation-panel"))
        .wait(Duration::from_secs(10), Duration::from_millis(500))
        .first()
        .await?;
    ...
    let sliders = ctx
        .driver
        .find_all(By::ClassName("ssg-chart-slider"))
        .await?;
    ...
    let slider_rows = ctx
        .driver
        .find_all(By::ClassName("chart-slider-row"))
        .await?;
    let sales_row_text = slider_rows[0].text().await?;
    assert!(
        sales_row_text.contains("15.0%"),
        "Sales CAGR display should show updated value"
    );
...
async fn test_sales_cagr_slider_controls_sales_projection() -> Result<()> {
    ...
    assert!(
        updated_text.contains("25.0%"),
        "Sales CAGR should show 25.0%, got: {}",
        updated_text
    );
...
async fn test_eps_cagr_slider_controls_eps_projection() -> Result<()> {
    ...
    assert!(
        updated_text.contains("18.5%"),
        "EPS CAGR should show 18.5%, got: {}",
        updated_text
    );
...
async fn test_sliders_independent_no_cross_contamination() -> Result<()> {
    ...
    assert_eq!(
        initial_eps_text, eps_after_sales_change,
    ...
    assert_eq!(
        sales_after, sales_after_eps_change,
...
async fn test_pe_sliders_affect_valuation_targets() -> Result<()> {
    ...
    assert!(
        pe_sliders.len() >= 2,
        "Should have High P/E and Low P/E sliders"
    );
    ...
    assert_ne!(
        initial_sell, updated_sell,
        "Sell zone should change when High P/E is adjusted"
    );
    ...
    assert_ne!(
        initial_buy, updated_buy,
        "Buy zone should change when Low P/E is adjusted"
    );
...
async fn test_command_strip_navigation_all_pages() -> Result<()> {
    ...
    assert!(
        nav_links.len() >= 3,
        "Should have at least 3 nav links (Home, System Monitor, Audit Log)"
    );
    ...
    let sys_page = ctx
        .driver
        .query(By::ClassName("system-monitor-page"))
        .wait(Duration::from_secs(5), Duration::from_millis(500))
        .first()
        .await?;
    assert!(
        sys_page.is_displayed().await?,
        "System Monitor page should load"
    );
    ...
    let audit_page = ctx
        .driver
        .query(By::ClassName("audit-log-page"))
        .wait(Duration::from_secs(5), Duration::from_millis(500))
        .first()
        .await?;
    assert!(
        audit_page.is_displayed().await?,
    ...
    let search = ctx
        ...
        .await?;
    assert!(
        search.is_displayed().await?,
        "Home page search bar should load"
    );
...
async fn test_direct_url_navigation() -> Result<()> {
    ...
    assert!(
        sys_header.text().await?.contains("SYSTEM"),
        "Direct /system-monitor navigation should work"
    );
    ...
    assert!(
        audit_header.text().await?.contains("AUDIT"),
        "Direct /audit-log navigation should work"
    );
    ...
    let command_strip = ctx
        .driver
        .query(By::ClassName("command-strip"))
        .wait(Duration::from_secs(5), Duration::from_millis(500))
        .first()
        .await?;
    assert!(
        command_strip.is_displayed().await?,
...
async fn test_keyboard_navigation_basics() -> Result<()> {
    ...
    assert!(
        val.contains("AAPL"),
        "Search input should accept keyboard input"
    );

    // Verify Tab moves focus (check that active element changes)
    let initial_active: String = ctx
        .driver
        .execute("return document.activeElement.className;", vec![])
        .await?
        .json()
        .as_str()
        .unwrap_or("")
...
async fn test_override_modal_keyboard_dismiss() -> Result<()> {
    ...
    loop {
        ...
        assert!(
            dismiss_start.elapsed() < dismiss_timeout,
            "Modal should be dismissed by Escape key within {:?}",
            dismiss_timeout
        );
...
async fn test_thesis_lock_modal_keyboard_dismiss() -> Result<()> {
    ...
    let lock_btn = ctx
        .driver
        .find(By::XPath("//button[contains(text(), 'Lock Thesis')]"))
        .await?;
    ...
    loop {
        ...
        assert!(
            dismiss_start.elapsed() < dismiss_timeout,
            "Thesis modal should be dismissed by Escape key within {:?}",
            dismiss_timeout
        );
        tokio::time::sleep(Duration::from_millis(100)).await;
    }
...
async fn test_thesis_lock_persists_after_navigation() -> Result<()> {
    ...
    let confirm_btn = modal
        .find(By::XPath(
            ".//button[contains(text(), 'Lock Permanent Snapshot')]",
        ))
        .await?;
    ...
    let snapshot = ctx
        .driver
        .query(By::ClassName("snapshot-header"))
        .wait(Duration::from_secs(5), Duration::from_millis(500))
        .first()
        .await?;
    ...
    assert!(
        search.is_displayed().await?,
        "Home page should load after navigation round-trip"
    );

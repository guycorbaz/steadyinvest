...
> **DONE — walked by Guy on 2026-09-22/23** (A: 5/5 · B: 10/10 · C: 6/6, C5 not triggered).
> Gaps found → PR #205 (native pickers, « dossier » wording, Portefeuille/Liste de suivi scroll),
> issue #207 (PDF vs the NAIC form), issue #208 + spec PR #206 (entry dialogs, acknowledged
> refusals, panel cards — blocks 7.1/7.2). Gates F1 + F6 are closed; this file is a record.
...
- [x] **5.1 Confront** — open a past study → « Confronter » : the recorded projection band + the
  actual price trajectory render; close cleanly.
- [x] **5.2/5.3 Export/Import (Réglages)** — export one study; export the whole journal; re-import
  each into a scratch journal; counts + notices correct.
- [x] **5.4 Backup/Restore** — create a backup; restore it; the confirm banner shows the
  (journal_id, version) and stale/foreign warnings when applicable.
- [x] **5.5 Journal location** — the RECENT-JOURNALS list; switch journals; the single-instance
...
- [x] **5.6 PDF** — « Exporter PDF » from a study; open the file; greyscale-readable; all
  sections expanded.

## B. Epic-6 batch

- [x] **6.1 Portfolios** — add a second bank; the selector chips; rename; the guarded deletes
  (has-holdings refusal, last-portfolio refusal).
- [x] **6.2 Currency** — the holding currency picker (allow-list only); per-row currency labels;
  per-currency CaR lines never mixing currencies.
- [x] **6.3 Ledger** — open a holding's Transactions; record a buy, a partial sell (position
  stays, average recomputes), edit and delete a row; the oversell refusal.
- [x] **6.4 Dividende** — record one with an empty « Retenue » (auto 35 %); the net line; the
  reinvestable-cash block updates.
- [x] **6.5 FX (Réglages)** — manual rate entry (bad rate/date refusals); « Actualiser les
  taux » fetches one pair per foreign currency in use.
- [x] **6.6 Consolidation** — with two banks + a foreign holding: per-bank lines, « Total
...
- [x] **6.7 Concentration** — the per-security shares (one line for a ticker held at two banks);
...
- [x] **6.8 Candidats** — sell a holding (partial AND whole): the panel auto-opens headed
  ...
  panel gone.
- [x] **6.9 Repli (Réglages)** — the three fallback rows; the fundamentals row has NO Twelve
  Data chip; pick a fallback = the current primary → the chip shows « Aucun repli » (the
  effective chain dedups).
- [x] **#94 fix** — edit a study's judgment (zone flips), navigate to Portefeuille: the zone
  chip/candidates are CURRENT on arrival (no unrelated mutation needed).

## C. Live-keys session (retro F6 — network + real keys)

- [x] **EODHD end-to-end** — key saved + « Tester » OK; a study fetch fills cells; a holdings
  price refresh stamps « à jour ».
- [x] **Twelve Data end-to-end** — same (remember its symbol convention differs: `NESN` not
  `NESN.SW` — issue #70; note what you observe).
- [x] **Forced fallback** — set the price fallback, then DELETE the primary's key: a price
...
- [x] **Pacing** — a Twelve Data refresh of >8 linked tickers: no quota error (requests spaced
  7.5 s — the batch takes ~1 min; note the UI feedback gap, issue #100).
- [x] *(not triggered — no quota hit)* **Quota reality (#80)** — if you hit a quota, note whether the provider actually sends a
  retry-after (the adapters never populate `retry_after_secs` today).
- [x] **FX live** — « Actualiser les taux » on both providers; the stored rows carry the right
  source wire name.

...
- [x] Update `HistoricalYearlyData` and `HistoricalData` in `steady-invest-logic` (AC: 4)
  - [x] Add `is_split_adjusted` field to `HistoricalData`.
  - [x] Add `adjustment_factor` to `HistoricalYearlyData` (internal use).
- [x] Implement Adjustment Logic in `steady-invest-logic` (AC: 3)
...
- [x] Verify Adjustment Correctness (AC: 3)
  - [x] Unit test in `steady-invest-logic` for the adjustment math.
...
- **Source Paths**:
  - Logic: `crates/steady-invest-logic/src/lib.rs`
  - Backend: `backend/src/services/harvest.rs`
  - Frontend: `frontend/src/pages/home.rs` (or dedicated component)

### Project Structure Notes

- Keep adjustment math in `steady-invest-logic` to ensure consistency between backend (storage) and frontend (rendering).
...
- Applied ADVERSARIAL Code Review findings:
  ...
  - Removed intermediate rounding in `steady-invest-logic` to preserve decimal precision during adjustment.

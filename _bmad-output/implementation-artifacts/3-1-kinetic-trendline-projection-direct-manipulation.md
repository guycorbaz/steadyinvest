...
5. **High-Precision Logic:** Calculation of the projected CAGR and trendline points must be performed in the `steady-invest-logic` crate for consistency.
...
- [x] Implement Kinetic Logic (AC: 2, 3)
  ...
  - [x] Implement `calculate_projection_from_point` in `steady-invest-logic`.
...
- [ ] Verification
  ...
  - [ ] Unit test: Verify projection math in `steady-invest-logic`.
...
- **Source tree components to touch:**
  ...
  - `crates/steady-invest-logic/src/lib.rs`: Projection math.
- **Testing standards summary:** Verify that dragging past the chart boundaries is handled gracefully.

### Project Structure Notes

- Alignment with unified project structure: Logic is kept in `steady-invest-logic`, UI in `frontend`.
...
- Implemented `calculate_projected_trendline` in `steady-invest-logic`.
...
- `crates/steady-invest-logic/src/lib.rs`

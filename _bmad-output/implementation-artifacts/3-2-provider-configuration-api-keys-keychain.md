...
### Post-review fix (manual GO/NO-GO finding, 2026-06-25)

During the manual GO/NO-GO, a valid free-tier EODHD key was reported as "clé invalide". Root cause: EODHD returns **HTTP 403** ("Only EOD data allowed for free users") for `/fundamentals` on the free plan, and the adapter mapped `401 | 403 → InvalidOrAbsentKey` — telling the user a *valid* key is invalid (a wrong signal the project forbids). Fix: split **401 → InvalidOrAbsentKey** vs **403 → new `ProviderError::Forbidden { detail }`** (carries the provider's own reason, key-free), surfaced in the key-test as the honest neutral `MSG_KEY_FORBIDDEN` ("La clé est valide, mais l'abonnement ne couvre pas ces données…"). Confirms the keychain GO/NO-GO **PASSES** (key stored in the OS secret store, read back, valid per EODHD `/user` → 200); the auto-fetch of fundamentals needs a paid EODHD plan (product decision, deferred). Partially addresses the test-cause nuance in issue #42. Files: `ingestion/src/{error,adapters/eodhd}.rs`, `app/src/{state,main,posture}.rs`. Gates green (app 158, full workspace, clippy/fmt/deny).

...
fn main() -> Result<(), slint::PlatformError> {
    ...
    let quick_screen: Rc<RefCell<Option<wiring::quick_screen::QuickScreenSession>>> =
    ...
    // journal / unreadable configured file). Pushed before the window shows.
    {
        ...
        if let Some(notice) = &startup_notice {
            // Through the list slot's F4 owner (G1 P): a standing state, kept as a failure is.
            wiring::list_notice::fail(&ui, wiring::list_notice::Source::Startup, notice);
        }

...
/// - otherwise (the startup dossier: no switch recorded it) → the sync-folder warning when it
///   applies, else nothing.
///
...
fn status_after_refusal(
    ...
    match recorded {
        ...
        _ if in_sync_folder => state::MSG_SYNC_FOLDER_WARNING.to_string(),
        _ => String::new(),
    }
}

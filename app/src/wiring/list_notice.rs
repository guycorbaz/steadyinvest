...
//!
//! Some writers of this slot do not go through here yet (the startup notice, a fetch or an
//! examination result for a study that is not open — fetch.rs is moved onto this helper by its
//! own branch). A notice on show that this module did not write has no known kind, so it is
...
pub(crate) enum Source {
    ...
    StudyAction,
    /// A provider fetch whose study is not open (fetch.rs, when it moves onto this helper).
    #[allow(dead_code)] // wired by the fetch.rs list-slot branch
    Fetch,
}
